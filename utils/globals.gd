...
func set_subtitle(
	index, 
	text, 
	before_color: Color = Color("#4682b4"), 
	current_color: Color = Color("#4682b4"), 
	after_color: Color = Color("#4682b4")
):
	# Correct chars. 
	...
	# Chars left to type.  
	var todo_chars = get_bbcode_color_tag(after_color) + text.substr(index+1, text.length()-1) + get_bbcode_color_tag()
